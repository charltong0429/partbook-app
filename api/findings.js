...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Server-side only (Vercel function logs) — never included in the
    // response, which stays the generic findings_unavailable message.
    console.error(`[api/findings] ${error.status ?? 500}: ${error.message}`);
